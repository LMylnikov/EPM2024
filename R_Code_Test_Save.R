...
i <- 992
N <- 12
S12<-S_Prob(N, 23, 1000) #стартовое для id вместо  1000         ->     id + шаг *0
S3213<-S_Periodic(N, 42, 1000) #стартовое для id + шаг для id * (номер S по счету -1) вместо 1000          ->       id + шаг *1
S3<-S_Prob(N, 0,5, 1000) #стартовое для id + шаг для id * (номер S по счету -1) вместо 1000     ->          id + шаг *2
